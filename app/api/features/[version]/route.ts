	...
	try {
		const maxAge =
			params.version === "nightly" || params.version === "beta"
				? 60 * 15
				: 60 * 60 * 24 * 7;
		const cargoRequest = await fetch(
			`https://doc.rust-lang.org/${params.version}/cargo/reference/unstable.html`,
			{
				cache: "no-store",
			},
		);
		...
		try {
			const bookRequest = await fetch(
				`https://doc.rust-lang.org/${params.version}/unstable-book/toc.html`,
				{
					cache: "no-store",
				},
			);
			...
			const bookRequest = await fetch(
				`https://doc.rust-lang.org/${params.version}/unstable-book/the-unstable-book.html`,
				{
					cache: "no-store",
				},
			);
